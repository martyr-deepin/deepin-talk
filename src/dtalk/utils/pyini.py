...
class Section(SortedDict):
    ...
    def dumps(self, out, convertors=None):
        ...
        if self._root and self._root._raw:
            print_('%s [%s]' % (self._info, self._name), file=out)
        else:
            print_('[%s]' % self._name, file=out)
        ...
        for f in self.keys():
            ...
            if self._root and self._root._raw:
                print_("%s %s%s" % (str(self[f]), f, self[f].value()), file=out)
            else:
...
class Ini(SortedDict):
    ...
    def save(self, filename=None):
        ...
        print_('#coding=%s' % self._encoding, file=f)
        for s in self.keys():
            if s in self._env:
                continue
            section = self[s]
