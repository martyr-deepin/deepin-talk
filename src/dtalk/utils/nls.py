...
try:
    ...
    try:
        locale.textdomain(PROGRAM_NAME)
    except AttributeError: # E.g. Windows
        pass
    # Required for dynamically added messages
    gettextmod.textdomain(PROGRAM_NAME)

    if xdg.local_hack: # running from source dir, so we have to set the paths
        import os.path
        locale_path = os.path.join(xdg.program_dir, 'locale')
        try:
            locale.bindtextdomain(PROGRAM_NAME, locale_path)
        except AttributeError: # E.g. Windows
            pass
        gettextmod.bindtextdomain(PROGRAM_NAME, locale_path)
